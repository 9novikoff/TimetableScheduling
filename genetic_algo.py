...
# subjects_constraint = {'linalg': ['teacher1'], 'calculus': ['teacher2'], 'db': ['teacher3']}
# group_constraint = {'tk-41': {'linalg': 5, 'calculus': 5, 'db': 5}, 'tk-42': {'linalg': 5, 'calculus': 5, 'db': 5}}
# teacher_constraint = {'teacher1': 10, 'teacher2': 10, 'teacher3': 10}

def visualize_timetable(timetable, groups):
    ...
    for day in range(days):
        for subject_number in range(max_subjects_per_day):
            table_data.append([timetable[group][day][subject_number][0] + ' ' + timetable[group][day][subject_number][1] for group in groups])
...
def fitness_function(individual, days, max_subjects):
    ...
    teachers_multiple_groups_constraint = {teacher: timetable for teacher, timetable in zip(individual_teacher_constraint.keys(), [[[False for j in range(max_subjects)] for i in range(days)] for teacher in range(len(individual_teacher_constraint.keys()))] )}
...
def mutate(individual, subjects, subject_constraint, days, max_subjects):
    ...
    mutated_individual[group][day][subject_number] = (mutation, np.random.choice(subject_constraint[mutation], 1)[0]) if mutation != '' else ('', '')

    return mutated_individual


def genetic_algorithm(population_size, generations, mutation_rate, subjects, subjects_constraint, group_constraint, teachers_constraint, days, max_subjects):
    population = initialize_population(population_size, subjects, subjects_constraint, group_constraint, days, max_subjects)

    for generation in range(generations):
        fitness_scores = [fitness_function(individual, days, max_subjects) for individual in population]

        selected_parents = np.random.choice(population, size=population_size * 2 // 3, p=fitness_scores / np.sum(fitness_scores), replace=True)
...
solutions = genetic_algorithm(pop_size, gen_count, mut_rate, subjects, subjects_constraint, group_constraint, teacher_constraint, days, max_subjects_per_day)

res = sorted(solutions, key=lambda x: fitness_function(x, days, max_subjects_per_day), reverse=True)[:10]

for r in res:
    print(r, fitness_function(r, days, max_subjects_per_day), '\n')

visualize_timetable(res[0], groups)
